...
JHipster 的学习和使用
JHipster 或者称 Java Hipster，是一个应用代码产生器，能够创建 spring Boot + AngularJS + Bootstrap 的应用。
...
安装过程要选择一些配置，见 Creating an application 。 
...
ibase.exception.ChangeLogParseException: Error Reading Migration File: class path resource [config/liquibase/master.xml] cannot be resolved to URL because it does not exist  


将项目打包通过 ssh 远程部署项目，启动，报下面的异常： 
...
如果  CacheConfiguration.java 或者 master.xml 等文件不能自动注入，则是因为代码中的注释被删除。
